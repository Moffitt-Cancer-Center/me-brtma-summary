...
There is limited clinical information used in this paper for the cohort, so instead of a typical Table 1, we provide an overview of all of the scores used in the paper separated out by cohort (@tbl-overall-summary).
...
p <- ggplot2::ggplot(
  ...
  ggplot2::labs(caption = "p = 0.066, Fisher's exact test") +
  # ggpubr gives a clean theme
  ggpubr::theme_pubr() +
  # Add space to x axis (left bar) and limit y axis to 0-1 range 
  # (no additional space).
